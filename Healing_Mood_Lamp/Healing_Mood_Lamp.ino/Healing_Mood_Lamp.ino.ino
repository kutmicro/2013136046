...
void turnRGB(){
  ...
  //그에 led 밝기는 0에서 최소, 255에서 최대가 됨(low,high) 타입에따라 반대가 될 수 있음
}
...
void loop() {
  play_state = digitalRead(busy);                       // mp3모듈의 busy 핀에서 나오는 신호를 판독
 if(play_state == HIGH && firstmusicorder == true ){    // 음악을 한번이상 재생한적이 있고, 음악이 재생이 끝났다면
    mp3_next ();                                        // 다음곡 재생
  }
